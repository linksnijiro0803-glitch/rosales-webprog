...
import fallbackImage from "/src/assets/images/typewriter.jpg";
  ...
  return (
    <div className="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
      {articles.map((article, index) => (
        <article
          key={article._id || article.name || article.slug}
        ...
        >
          ...
          <div className="p-4">
            ...
            <p className="mt-3 text-sm leading-6 text-[#3A3534]">
              {(article.preview || article.content?.[0] || article.paragraph || "").substring(0, 150)}...
            </p>

            <Link to={`/articles/${article.name || article.slug}`}>
